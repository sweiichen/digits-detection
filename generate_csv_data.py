...
def get_img_boxes(f, annotations, idx=0):
    ...
    obj_count = 0
    for left, top, width, height, label in zip(ann['left'], ann['top'], ann['width'], ann['height'],ann['label']):
        meta['x0'] = left
        meta['y0'] = top
        meta['x1'] = left + width
        meta['y1'] = top + height
...
annotations = pd.DataFrame(columns=["filename", "width", "height", "x0", "y0", "x1", "y1", "label"])
